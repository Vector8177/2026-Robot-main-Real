...
public class ShooterVisualizer {
    ...
    public void setRobotPoseSupplier(Supplier<Pose2d> robotPoseSupplier){
        ...
        fuelLaunchPointSupplier = () -> {
            return new Translation3d(
                robotPoseSupplier.get().getX() + SHOOTER_PIVOT.getX(), 
                robotPoseSupplier.get().getY() + SHOOTER_PIVOT.getY(), 
                SHOOTER_PIVOT.getZ());
        };
    }
    ...
    public void setFlywheelAngularVelocitySupplier(Supplier<AngularVelocity> shooterAngularVelocitySupplier){
        this.fuelVelocitySupplier = () -> {
            ...
            return MetersPerSecond.of(v.in(RadiansPerSecond) * FLYWHEEL_TO_LAUNCHER_GEARING * SHOOTER_WHEEL_RADIUS.in(Meters));
        };
    }
    
    public void launchFuel(ShooterIO io){
    ...
    public LinearVelocity toLinearVelocity(AngularVelocity velocity){
        return MetersPerSecond.of(velocity.in(RadiansPerSecond));
    }
}
